...
import { AuthUser, SessionPayload, useAuthStore } from "@/lib/auth-store";
...
/** Compte : double authentification + changement de mot de passe. */
export default function AccountPage() {
  ...
  const changePassword = useMutation({
    mutationFn: () =>
      api<SessionPayload>("/auth/change-password", {
        method: "POST",
        body: JSON.stringify({
    ...
    onSuccess: (session) => {
      ...
      setPasswordFeedback({ tone: "success", text: tSettings("password.success") });
    },
    onError: (caught) =>
      setPasswordFeedback({ tone: "error", text: errorText(caught) }),
  });
      ...
      <Card>
        <div className="mb-4 flex items-center gap-2 text-muted">
          ...
          <h2 className="text-sm font-semibold uppercase tracking-wider">
            {tSettings("password.title")}
          </h2>
        </div>
        {passwordFeedback && (
          <div className="mb-4">
            <Alert tone={passwordFeedback.tone}>{passwordFeedback.text}</Alert>
          </div>
        )}
        <form onSubmit={submitPassword} className="space-y-4">
          <div className="grid gap-3 sm:grid-cols-2">
            <div>
              <Label htmlFor="currentPassword">{tSettings("password.current")}</Label>
              <Input
                id="currentPassword"
                type="password"
            ...
            <div>
              <Label htmlFor="newPassword">{tSettings("password.new")}</Label>
              <Input
                id="newPassword"
                type="password"
